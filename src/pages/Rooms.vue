	...
	<div>
		...
		<div class="ui grid centered">
			<table class="ui very basic table">
				...
				<tbody>
					<tr v-for="element in filteredResults.slice()" :key="element.uid">
						<!-- <td>{{ device.id }}</td> -->
						<td data-label="Name">{{ element.name }} - {{ element.uid }}</td>
						<td data-label="Room Number">{{ element.roomId }}</td>
						<td data-label="Grade">{{ element.roomType }}</td>
						<!-- <td data-label="Box">{{ device.box }}</td> -->
						<td data-label="Location">{{ element.location }}</td>
						<td>
							<button @click="deleteDevice(element)" :key="element.uid">
								Remove
							</button>
						</td>
					</tr>
...
export default {
	...
	methods: {
		...
		deleteDevice: function (room) {
			if (confirm(`Are you sure you want to remove ${room.uid}?`)) {
				db.collection(`rooms`).doc(room.uid).delete();
			}
		},
		...
		filteredResults: function () {
			return this.rooms.filter((room) => {
				return (
					...
					room.uid.toLowerCase().indexOf(this.query.toLowerCase()) >= 0 ||
					room.roomType.toLowerCase().indexOf(this.query.toLowerCase()) >= 0 ||
					room.location.toLowerCase().indexOf(this.query.toLowerCase()) >= 0
				);
			});
		},
	},
