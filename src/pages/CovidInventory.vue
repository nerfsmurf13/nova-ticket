      ...
      <form v-on:submit.prevent class="ui huge form segment">
        ...
        <div class="field">
          <div class="field" :class="{ error: rfIdValid == 2 }">
            <label>RF #</label>
            <input type="text" v-model.trim="rfId" ref="step1" />
          </div>
...
export default {
  ...
  firestore: {
    devices: db.collection(`c19devices`),
  },
  updated() {
    this.log('Updated');
    this.isRfValid();
    ...
    submitDevice() {
      if (this.schoolCode == this.verifySchoolCode) {
        if (
          ...
          //testing
          this.logs.push({
            message: `Success! ${this.rfId} added!`,
            color: 'green',
          });
          this.rfId = '';
          // this.studentId = '';
